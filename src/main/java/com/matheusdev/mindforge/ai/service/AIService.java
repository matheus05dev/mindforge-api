...
public class AIService {
    ...
    public ChatMessage analyzeCodeForProficiency(CodeAnalysisRequest request) throws IOException {
        ...
        Optional<UserProfileAI> userProfile = memoryService.getProfile(userId);
        String profileSummary = userProfile.map(UserProfileAI::getSummary).orElse("Nenhum perfil ainda definido.");
        ...
        if (aiResponse.getError() != null) {
            throw new RuntimeException("AI Service Error: " + aiResponse.getError());
        }
    ...
    public ChatMessage analyzeGitHubFile(GitHubFileAnalysisRequest request) throws IOException {
        ...
        UserIntegration integration = userIntegrationRepository.findByUserIdAndProvider(userId, UserIntegration.Provider.GITHUB)
                .orElseThrow(() -> new RuntimeException("Usuário não conectou a conta do GitHub"));
        String accessToken = integration.getAccessToken();
        String repoUrl = project.getGithubRepoUrl();
        if (repoUrl == null || repoUrl.isEmpty()) {
            throw new RuntimeException("O projeto não está vinculado a um repositório do GitHub.");
        }
        ...
        Subject subject = subjectRepository.findAll().stream().findFirst()
                .orElseThrow(() -> new RuntimeException("Nenhum assunto de estudo encontrado para contextualizar a análise."));
    ...
    public KnowledgeItemResponse transcribeImageAndAppendToKnowledgeItem(Long documentId, Long itemId) throws IOException {
        ...
        if (document.getFileType() == null || !document.getFileType().startsWith("image")) {
            throw new IllegalArgumentException("O documento fornecido não é uma imagem.");
        }
    ...
    public ChatMessage reviewPortfolio(PortfolioReviewRequest request) throws IOException {
        ...
        String repoName = urlParts[1];
        UserIntegration integration = userIntegrationRepository.findByUserIdAndProvider(userId, UserIntegration.Provider.GITHUB)
                .orElseThrow(() -> new RuntimeException("Usuário não conectou a conta do GitHub para esta operação."));
    ...
    private String getCodeFromRequest(CodeAnalysisRequest request) throws IOException {
        ...
        throw new IllegalArgumentException("No code or document ID was provided for analysis.");
    }
